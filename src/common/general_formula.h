...
public:
   ...
   typedef typename VTSet::iterator iterator;
   ...
   GeneralFormula operator+ (const GeneralFormula& gf) const
   {
      ...
      if (_size > gf._size)
      {
         ...
         for (typename VTSet::iterator it=gf._vtset.begin() ; it!=gf._vtset.end() ; 
                         it++)
         {
            VarType val2 = *it;
      ...
      } else
      {
         ...
         for (typename VTSet::iterator it=_vtset.begin() ; it!=_vtset.end() ; it++)
         {
            VarType val2 = *it;
            itf = temp._vtset.find(val2);
            if (itf != temp._vtset.end())  // found
   ...
   {
      typename VTSet::iterator itf;
      for (typename VTSet::iterator it=gf._vtset.begin() ; it!=gf._vtset.end() ; it++)
      {
         VarType val2 = *it;
   ...
   GeneralFormula& operator*= (const coeff_t factor)
   {
      ...
      typename VTSet::iterator it=_vtset.begin();
      VTSet temp;
      while (it!=_vtset.end())
      {
         VarType val2 = *it;
         val2 *= factor;
         typename VTSet::iterator itf = it;
         it++;
         _vtset.erase(itf);
         temp.insert(val2);
      } 
   ...
   virtual void PrintObject (std::ostream &os) const
   {
      ...
      if (_formula_access_mode & ACCESS_INDIRECT)
         ...
         if (! _indirect_pcs.empty())
         {
            ...
            for (AddrIntSet::iterator sit=_indirect_pcs.begin() ; 
