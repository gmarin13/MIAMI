...
/* The following routines perform data flow analysis on memory operands to 
 * compute symbolic formulas that describe the accessed memory locations.
 */
uint64_t 
PrivateRoutine::SaveStaticData(FILE *fd)
{
   return (SaveStaticAnalysisData(fd, *rFormulas));
}

void 
PrivateRoutine::FetchStaticData(FILE *fd, uint64_t offset)
{
   LoadStaticAnalysisData(fd, offset, *rFormulas);
}

void
PrivateRoutine::ComputeBaseFormulas (ReferenceSlice *rslice, PrivateCFG *g, RFormulasMap& refFormulas)
{
   ...
   while (! nqueue.empty())
   {
      ...
      PrivateCFG::OutgoingEdgesIterator oeit(node);
      while((bool)oeit)
      {
         PrivateCFG::Node *b = oeit->sink();
         if (! b->visited(mm))
         {
            ...
            if (_loadedFormula.is_uninitialized())
            {
               ...
               for (sit=_loadedFormula.begin() ; sit!=_loadedFormula.end() ; ++sit)
               {
                  ...
                  if (tt == TY_LOAD)
                  {
                     ...
                     PrivatePrivateCFG::Node *find_b = g->findNodeContainingAddress(tpc);
                     assert (find_b != NULL);
...
bool
PrivateRoutine::ClarifyIndirectAccessForRef(RefStrideId& rsi, GFSliceVal& _formula, 
...
void
PrivateRoutine::ComputeStrideFormulasForScope(StrideSlice &sslice, ReferenceSlice *rslice, 
        ...
        RFormulasMap& refFormulas, RefInfoMap& memRefs)
{
   ...
   int kid;
   PrivateCFG *cfg = ControlFlowGraph();
   PrivateCFG::NodeList nqueue;
   PrivateCFG::Node *root_b = static_cast<PrivateCFG::Node*>(mCfg->GetRIFGNode(node));
//   addrtype loopStart = root_b->getStartAddress();
                ...
                kid = tarj->TarjNext(kid))
   {
      ...
      {
         PrivateCFG::Node *b = static_cast<PrivateCFG::Node*>(mCfg->GetRIFGNode(kid));
         if (b->Size() > 0)
            nqueue.push_back(b);
      }
   }
   ...
   while (! nqueue.empty())
   {
      PrivateCFG::Node *b = nqueue.front();
      nqueue.pop_front();
...
void
PrivateRoutine::ComputeStrideFormulasForRoutine(RIFGNodeId node, TarjanIntervals *tarj, 
            MiamiRIFG* mCfg, int marker, int level, ReferenceSlice *rslice)
{
   PrivateCFG::Node* b;
   int kid;

   b = static_cast<PrivateCFG::Node*>(mCfg->GetRIFGNode(node));
   b->markWith(marker);
                ...
                kid = tarj->TarjNext(kid))
   {
      if (! tarj->NodeIsLoopHeader(kid))
      {
         b = static_cast<PrivateCFG::Node*>(mCfg->GetRIFGNode(kid));
         // nothing to do here
         b->markWith(marker);
      }
